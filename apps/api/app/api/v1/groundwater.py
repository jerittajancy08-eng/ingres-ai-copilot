...
def query(payload: GroundwaterQueryRequest) -> dict[str, object]:
    ...
    district = next((item for item in DISTRICTS if item["name"].lower() == (payload.district or "").lower()), None)
    answer = "Retrieved groundwater context is available for this query. Use /chat for full Gemini-powered reasoning."
    if district:
        answer = (
            f"{district['name']} is at {district['stress']} groundwater stress with a {district['trend']} water table "
            f"at {district['level_m']} m below ground level. Recommended action: {district['recommendation']} "
            f"Recharge priorities: {' '.join(district['recharge_recommendations'])} "
            f"Conservation priorities: {' '.join(district['conservation_recommendations'])}"
        )
    return {
        "query": payload.query,
