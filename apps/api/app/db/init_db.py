...
def init_db() -> None:
    ...
    with engine.begin() as connection:
        if engine.dialect.name == "sqlite":
            ...
            connection.exec_driver_sql("UPDATE users SET role = 'user' WHERE role IS NULL OR role = 'citizen' OR role = 'officer'")
            ...
            if "access_roles" not in document_columns:
                connection.exec_driver_sql("ALTER TABLE documents ADD COLUMN access_roles TEXT DEFAULT 'user'")
            connection.exec_driver_sql("UPDATE documents SET access_roles = 'user' WHERE access_roles IS NULL OR access_roles = ''")
